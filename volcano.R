...
colnames(b.wt.gskvdmso.volcano) <- c("ID","Log2FC",
                                     "pvalue")
...
key.vals.wt <- ifelse(rownames(topTags(tr.wt.gskvdmso, n="Inf")$table) 
                ...
                ifelse(rownames(topTags(tr.wt.gskvdmso, n="Inf")$table)
                       %in% unname(mapIds(org.Mm.eg.db,
                            keys=Mm.hallmark$HALLMARK_MYC_TARGETS_V1, 
...
names(key.vals.wt)[key.vals.wt=='blue'] <-"IL6"
#names(key.vals.wt)[key.vals.wt=='purple'] <- "Myc"
names(key.vals.wt)[key.vals.wt=='red'] <- "INFLA"
names(key.vals.wt)[key.vals.wt=='darkorchid'] <- "MYC"

tiff("wt_gskvdmso_volcano_HALLMARK.tiff",units="in",width=6,height=6,res=600)
                ...
                pointSize = 
                  ifelse(rownames(topTags(tr.rlr.gskvdmso, n="Inf")$table) %in% 
                           unname(mapIds(org.Mm.eg.db,keys=Mm.hallmark$HALLMARK_IL6_JAK_STAT3_SIGNALING , 
                                         keytype = "ENTREZID", column="SYMBOL")),2.5,
                         ifelse(rownames(topTags(tr.rlr.gskvdmso, n="Inf")$table)%in% 
                                  unname(mapIds(org.Mm.eg.db,keys=Mm.hallmark$HALLMARK_INFLAMMATORY_RESPONSE,
                                                keytype = "ENTREZID", column="SYMBOL")),2.5,
                                ifelse(rownames(topTags(tr.rlr.gskvdmso, n="Inf")$table)%in% 
                                         unname(mapIds(org.Mm.eg.db,keys=Mm.hallmark$HALLMARK_MYC_TARGETS_V1,
                                                       keytype = "ENTREZID", column="SYMBOL")),2.5,0.4))),
